...
cistarget_res <- read_tsv('/lustre/scratch126/cellgen/team205/is10/fibroblasts/bulk_cytokines/cistarget/enrichment_padj0.01/cistarget_res_df_NES0.1.txt') %>%
  rename(motif=X1)

cistarget_res$Region_set <- gsub('differential_peak_table_FDR05_annotated_matched_', '', cistarget_res$Region_set)
...
```{r}
## Export
write.table(cistarget_res_mat, '/lustre/scratch126/cellgen/team205/is10/fibroblasts/bulk_cytokines/cistarget/enrichment_padj0.01/cistarget_res_df_NES0.1_motif_matrix.txt', row.names = T, col.names = T, sep = '\t', quote = F)

write.table(cistarget_res_mat_anno, '/lustre/scratch126/cellgen/team205/is10/fibroblasts/bulk_cytokines/cistarget/enrichment_padj0.01/cistarget_res_df_NES0.1_TF_matrix.txt', row.names = T, col.names = T, sep = '\t', quote = F)
```
